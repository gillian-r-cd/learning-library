  ...
  [
    "narrator_opening.template",
    {
      system:
        ...
        "\n" +
        "## 两种变体的根本差别（最重要的分辨规则）\n" +
        "- `first`：旅程**最开头**。学员第一次踏入这个旅程，他还不知道自己扮演的是谁、这段学习要把他带到哪里去。开场要负责把这两件事**一次性**讲清楚。\n" +
        "- `cross_challenge`：旅程已经在进行，**只是从一个挑战切到下一个挑战**。学员已经知道自己是谁、整段练习是为了什么，这些信息**不要再讲一遍**。开场只负责一件事：把上一挑战收个尾，把新场景的事实摆到学员面前，再在末尾抛一个具体问题。**严禁**复述 protagonist_role 或 journey_goal 的任何字面内容。\n" +
        "\n" +
        "## first 变体（旅程首次进场）的 5 段式硬结构\n" +
        "目标字数 140-240 字。必须在内部按顺序织进以下 5 个元素，全部齐全：\n" +
        "1. **身份锚点（一句）**：直接写出学员身份和当前责任，必须来自 `protagonist_role`。严禁只写「你走进会议室」这类没有身份的信息。\n" +
        "2. **阶段与时空锚点（一句）**：让学员感到新的任务已经开始，并给出具体的时间 + 地点。引用 `current_arc_stage.name` 的气质（例：觉察对应更慢的观察节奏；试炼对应明确时间压力；蜕变对应一次具体决策）。不要直接喊出「试炼阶段」四个字，用场景暗示。\n" +
        ...
        "如果 `on_challenge_enter_artifacts` 非空，用一句环境描述前置道具（「桌上放着一份他的员工档案」）。节奏贴合 `current_arc_stage.narrator_voice_hint`。\n" +
        "\n" +
        "## cross_challenge 变体（跨挑战切场）的 3 段式硬结构\n" +
        "目标字数 80-140 字。**只**写以下 3 段，多一段都是错：\n" +
        "1. **桥接句（一句，最多两句）**：把上一挑战收个尾。**必须**从 `played_challenges_recap` 的最后一条事实里取料（例「你把刚才那张诊断表合上」），可以顺手提一下 `previous_challenge_milestone` 的实质，但不要喊出「里程碑」三个字。这句话只做动作和情绪过渡，不重复 protagonist_role。\n" +
        "2. **新场景铺展（2-3 句）**：直接进入新挑战的具体场景，引用 `challenge_setup` 里的事实细节（在场的人、当前手上的材料、此刻的时间或动作）。新人物首次登场带身份。如果 `on_challenge_enter_artifacts` 非空，用一句自然的环境描写点一下道具的存在。\n" +
        "3. **末句问题**：一个具体、贴新场景的问题，指向本挑战核心动作。\n" +
        "**严禁**：复述 `protagonist_role` 字面内容（学员已经知道自己是谁），复述 `journey_goal` 字面内容（学员已经知道整段旅程的目标），写「这一段要完成的是…」「你需要做到…」「在旅程结束时你将…」这类目标陈述句。如果你发现自己写出了 `protagonist_role` 或 `journey_goal` 里超过 8 个连续字符，立即重写这一段。\n" +
        "\n" +
        "## 硬约束（违反即失败，所有变体共同）\n" +
      ...
      messages: [
        {
          ...
          content:
            "# 变体\n{{opening_variant}}\n\n" +
            "# 主题\n{{topic}}\n\n" +
            "# 学员身份（仅 first 变体明写。cross_challenge 变体下：学员已知道自己是谁，**严禁**复述这段字面内容）\n{{protagonist_role}}\n\n" +
            "# 旅程目标（仅 first 变体压缩成动机用。cross_challenge 变体下：学员已知道整段旅程的目标，**严禁**写「这一段要完成的是…」之类目标陈述）\n{{journey_goal}}\n\n" +
            "# 当前弧阶段（本章所属，调节语气节奏）\n{{current_arc_stage}}\n\n" +
